...
%define slurm_micro 11
%define slurm_version 14.11.11
%define slurm_release 1


Name:    slurm
Version: %{slurm_major}.%{slurm_minor}.%{slurm_micro}.%{slurm_release}
Release: 1%{?dist}
...
#https://github.com/SchedMD/slurm/archive/slurm-14-11-11-1.tar.gz
Source: %{name}-%{slurm_major}-%{slurm_minor}-%{slurm_micro}-%{slurm_release}.tar.gz
...
%prep
%setup -n %{name}-%{name}-%{slurm_major}-%{slurm_minor}-%{slurm_micro}-%{slurm_release}
...
#Clean rpaths
cd %{buildroot}
for x in `find . -type f`;do if [[ ! -z $(file $x | grep ELF) ]]; then if [[ ! -z $(chrpath -l $x | grep RPATH) ]]; then chmod u+w $x;  chrpath --convert $x; fi ; fi; done
%endif
#############################################################################
